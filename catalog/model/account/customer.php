...
class ModelAccountCustomer extends Model {
	...
	public function getCustomer($customer_id) {
		$query = $this->db->query("SELECT *, CONCAT('".HTTP_SERVER."image/',customer_image) AS customer_image, (SELECT cg.name from ".DB_PREFIX."customer_group_description cg WHERE cg.customer_group_id=customer_group_id LIMIT 1) AS group_name, (SELECT CONCAT('".HTTP_SERVER."image/',cg1.group_image) from ".DB_PREFIX."customer_group_description cg1 WHERE cg1.customer_group_id=customer_group_id  LIMIT 1) AS group_image FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int)$customer_id . "'");

		return $query->row;
	}
	public function getLoginCustomer($email, $password) {
		$salt = $this->db->escape($salt = token(9));
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) . "' AND password = '".$this->db->escape(sha1($salt . sha1($salt . sha1($password))))."'");
