<?php
// Heading
$_['heading_title']			= 'Klarna konto';

// Text
$_['text_extension']		= 'Extensions';
$_['text_success']			= 'Framgång: Du har ändrat Klarna Payment module!';
$_['text_edit']             = 'Redigera Klarna konto';
$_['text_klarna_account']	= '<a href="https://merchants.klarna.com/signup?locale=en&partner_id=d5c87110cebc383a826364769047042e777da5e8&utm_campaign=Platform&utm_medium=Partners&utm_source=Opencart" target="_blank"><img src="https://cdn.klarna.com/public/images/global/logos/v1/basic/global_basic_logo_std_blue-black.png?width=60&eid=opencart" alt="Klarna Account" title="Klarna Account" style="border: 1px solid #EEEEEE;" /></a>';
$_['text_live']				= 'leva';
$_['text_beta']				= 'Beta';
$_['text_sweden']			= 'Sverige';
$_['text_norway']			= 'Norge';
$_['text_finland']			= 'finland';
$_['text_denmark']			= 'Danmark';
$_['text_germany']			= 'Tyskland';
$_['text_netherlands']		= 'Nederländerna';

// Entry
$_['entry_merchant']		= 'Klarna Merchant ID';
$_['entry_secret']			= 'Klarna Secret';
$_['entry_server']			= 'server';
$_['entry_total']			= 'Total';
$_['entry_pending_status']	= 'Väntar på status';
$_['entry_accepted_status'] = 'Godkänd status';
$_['entry_geo_zone']		= 'Geo Zone';
$_['entry_status']			= 'Status';
$_['entry_sort_order']		= 'Sorteringsordning';

// Help
$_['help_merchant']			= '(estore id) att använda för Klarna tjänsten (tillhandahållen av Klarna).';
$_['help_secret']			= 'Delad hemlighet att använda med Klarna-tjänsten (tillhandahållen av Klarna).';
$_['help_total']			= 'Kassa totalt ordern måste nås innan betalningsmetoden blir aktiv.';

// Error
$_['error_permission']		= 'Varning: Du har inte behörighet att ändra betalning Klarna Delbetalning!';
$_['error_pclass']			= 'Det gick inte att hämta pClass för %s. Felkod: %s; Felmeddelande: %s';
$_['error_curl']			= 'Curl Error - Kod: %d; Meddelande: %s';
$_['error_log']				= 'Det fanns fel att uppdatera modulen. Kontrollera loggfilen.';