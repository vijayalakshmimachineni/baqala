<?php
class ControllerSaleOrderstatus extends Controller {
	private $error = array();
	...
	protected function getList() {
		...
		$data['sort_delivery_date'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=o.delivery_date' . $url, true);
		...
		$payments = array();
		$this->load->model('sale/order');
		$res = $this->model_sale_order->getPaymentMethods();
		if(!empty($res)) {
			foreach ($res as $key => $value) {
				$l = $this->load->language('extension/payment/'.$value['code']);
				$payments[$value['code']]['code'] = $value['code'];
				$payments[$value['code']]['title'] = $this->language->get('heading_title');
			}
		}
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['delivery_time'] = date('H:i:s', strtotime($order_info['delivery_time']));
			$this->load->model('customer/customer');
